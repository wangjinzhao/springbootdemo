...
public class HttpClientUtil {
    ...
    private static int connectionRequestTimeout = 5000;//指从连接池获取连接的timeout
    private static int connectTimeout = 2000;//client 与 server 建链超时时间 （三次握手的时间)
    private static int socketTimeout = 1000;//指client与server 数据传输 超时时间


    static {
        //TODO 可以动态配置比如从zk读取或是从配置文件读取maxTotal   defaultMaxPerRoute connectionRequestTimeout connectTimeout socketTimeout
    ...
    public static CloseableHttpClient getHttpClient() {
        try {
            httpClient = HttpClients.custom()
                    ...
                    .setConnectionManagerShared(true)//设置长连接
...
//            RequestConfig requestConfig = RequestConfig.custom().setCookieSpec(CookieSpecs.STANDARD_STRICT).build();
    ...
    public static RequestConfig requestConfig() {
        RequestConfig requestConfig = RequestConfig.custom()
                ...
                .setConnectionRequestTimeout(HttpClientUtil.connectionRequestTimeout)
                .setConnectTimeout(HttpClientUtil.connectTimeout)
                .setSocketTimeout(HttpClientUtil.socketTimeout)
                .build();
    ...
    private static class PoolManager {
        ...
        public PoolManager() {
            ...
            clientConnectionManager.setDefaultMaxPerRoute(defaultMaxPerRoute);//将每个路由基础的连接设置为defaultMaxPerRoute
        }
        ...
        public static PoolingHttpClientConnectionManager getHttpPoolInstance() {
            ...
            //TODO 可以开启一个后台线程池对当前的http连接池做监控
    ...
    public static class PoolsManager {
        ...
        private PoolsManager() {
            clientConnectionManager.setMaxTotal(HttpClientUtil.maxTotal);
            clientConnectionManager.setDefaultMaxPerRoute(HttpClientUtil.defaultMaxPerRoute);
        }
